...
function updateSliderBackground(slider) {
  ...
  slider.style.background = `linear-gradient(to right, var(--primary) 0%, var(--primary) ${percentage}%, var(--neutral-200) ${percentage}%, var(--neutral-200) 100%)`;
}

updateSliderBackground(sizeInput);
updateSliderBackground(speedInput);
...
speedInput.addEventListener("input", () => {
  spinnerState.speed = parseFloat(speedInput.value);
  speedValueLabel.textContent = `${spinnerState.speed.toFixed(1)}s`;
  updateSliderBackground(speedInput);
  updateSpinner();
});
